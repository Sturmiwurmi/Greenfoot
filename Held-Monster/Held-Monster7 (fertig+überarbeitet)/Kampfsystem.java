...
public class Kampfsystem extends Actor
{
    ...
    private Held held =new Zauberer("_Name_",3,100);
    ...
    public void act(){
        ...
        // Es war erforderlich die nächsten 5 Zeilen in der Klasse Kampfsystem zu schreiben weil ich
        // mit der Klasse Held oder einer Vererbung von Held keine möglichkeit hab die Leben des Monsters
        //zu aktualliesieren
        ...
        if(heldgewählt == false){
            if(!knopf1.getheldausgewählt().equals("keiner")){
                ...
                if(heldausgewählt.equals("Zauberer")){
                    ...
                    myWorld.removeObjects(myWorld.getObjects(Knopf.class));
                    heldgewählt = true;
                }               
            }
    ...
    public void Lebenanzeigen(){
        if(heldgewählt){
            switch(Gewinner()){
                case "Niemand":
                    this.myWorld.showText(String.valueOf(lebenMonster), 194,180); //        
                    this.myWorld.showText(String.valueOf(lebenHeld), 476,130); 
                    break;
                case "Held":
                    this.myWorld.showText(String.valueOf("TOD"), 194,180);
                    this.myWorld.showText(String.valueOf("Sieger"), 476,130); 
                    break;

                case "Monster":
                    this.myWorld.showText(String.valueOf("Sieger"), 194,180);
                    this.myWorld.showText(String.valueOf("TOD"), 476,130); 
                    break;
            }
